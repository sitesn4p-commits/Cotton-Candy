...
function formatLkr(amount: number | undefined) { return new Intl.NumberFormat('en-AU', { style: 'currency', currency: 'AUD', maximumFractionDigits: 2 }).format(amount || 0) }
...
function OrderDetailDialog({ request, onClose, onStatusChange, onDelete, campaign }: { request: ServiceRequest; onClose: () => void; onStatusChange?: (request: ServiceRequest, status: RequestStatus) => void; onDelete: (request: ServiceRequest) => Promise<void>; campaign?: PromotionCampaignActions }) {
  ...
  const emailSection = campaign ? <PromotionEmailComposer request={request} campaign={campaign} /> : request.status === 'cancel' ? null : <ActivationEmailPreview request={request} />
...
export function AdminCancelledOrdersPage() {
  ...
  const [selectedRequest, setSelectedRequest] = useState<ServiceRequest | null>(null)
  const [error, setError] = useState<string | null>(null)
  const load = useCallback(() => api.adminServiceRequests(token, 'cancel').then((items) => { setRequests(items); setError(null) }).catch((reason: unknown) => setError(messageFor(reason, 'Unable to load cancelled orders.'))), [token])

  useEffect(() => { void load() }, [load])

  const deleteRequest = async (request: ServiceRequest) => {
    if (!await confirm({ title: 'Delete this cancelled order?', message: `${request.offeringName} for ${request.customerName || 'this customer'} will be permanently deleted. This cannot be undone.`, confirmLabel: 'Delete order', tone: 'error' })) return
    try {
      await api.deleteServiceRequest(token, request._id)
      setRequests((current) => current.filter((item) => item._id !== request._id))
      setSelectedRequest((current) => current?._id === request._id ? null : current)
      notify({ title: 'Cancelled order deleted', message: `${request.offeringName} has been removed from the cancelled order list.` })
    } catch (reason) { setError(messageFor(reason, 'Unable to delete cancelled order.')) }
  }

  return <section className="admin-page"><div className="admin-page-heading"><div><p className="eyebrow">Cancelled by customers</p><h1>Cancelled <em>orders.</em></h1></div></div><ErrorNotice error={error} /><section className="admin-card admin-table-card">{requests.length ? <div className="request-admin-list">{requests.map((request) => <article key={request._id}><button className="request-admin-summary" type="button" onClick={() => setSelectedRequest(request)}><strong>{request.customerName || 'Customer'}</strong><p>{request.offeringName} · <b>{formatLkr(request.totalPrice)}</b></p><small>{request.email}{request.eventDate ? ` · ${new Date(request.eventDate).toLocaleDateString()}` : ''}</small><span>View cancelled order →</span></button><div className="history-order-labels"><span className="request-status cancel">cancelled</span><button type="button" onClick={() => void deleteRequest(request)}>Delete</button></div></article>)}</div> : <EmptyState>Customer cancellations will appear here, ready to review or delete.</EmptyState>}</section>{selectedRequest ? <OrderDetailDialog request={selectedRequest} onClose={() => setSelectedRequest(null)} onDelete={deleteRequest} /> : null}</section>
}
...
export function AdminHeroArtworkPage() {
  const token = useAdminToken()
  const { notify } = useFeedback()
  const [content, setContent] = useState<HomeContent | null>(null)
  const [error, setError] = useState<string | null>(null)
  ...
  useEffect(() => { api.adminPageArtwork(token).then(setContent).catch((reason: unknown) => setError(messageFor(reason, 'Unable to load hero artwork.'))) }, [token])
  ...
  const submit = async (event: FormEvent<HTMLFormElement>, message: string) => {
    ...
    const form = event.currentTarget
    try {
      const updated = await api.updatePageArtwork(token, new FormData(form))
      setContent(updated)
      form.reset()
      notify({ title: 'Hero artwork updated', message })
    } catch (reason) {
      setError(messageFor(reason, 'Unable to update hero artwork.'))
    } finally {
      setSaving(false)
    }
  }

  const galleryHero = content?.galleryHeroUrl || 'https://images.unsplash.com/photo-1519167758481-83f550bb49b3?auto=format&fit=crop&w=1400&q=85'
  const sharedHero = content?.bookingsHeroUrl || 'https://images.unsplash.com/photo-1519167758481-83f550bb49b3?auto=format&fit=crop&w=1400&q=85'

  return <section className="admin-page"><div className="admin-page-heading"><div><p className="eyebrow">Public page images</p><h1>Hero artwork, <em>beautifully shared.</em></h1></div></div><ErrorNotice error={error} /><div className="admin-split"><Panel title="Gallery page hero"><form className="admin-form-grid" onSubmit={(event) => void submit(event, 'Your Gallery image is now live across the Gallery pages.')}><label className="file-input admin-full">Gallery hero image<input name="galleryHero" type="file" accept="image/*" required /></label><p className="admin-help admin-full">This image appears behind the Gallery Images and Gallery Videos page headings.</p><button className="admin-button" type="submit" disabled={saving}>{saving ? 'Uploading…' : 'Update Gallery hero'}</button></form></Panel><Panel title="Current Gallery hero"><div className="page-artwork-single"><img src={galleryHero} alt="Current Gallery hero" /></div></Panel></div><div className="admin-split"><Panel title="Bookings, Services and Hire hero"><form className="admin-form-grid" onSubmit={(event) => void submit(event, 'Your shared Bookings, Services and Hire hero image is now live.')}><label className="file-input admin-full">Shared hero image<input name="bookingsHero" type="file" accept="image/*" required /></label><p className="admin-help admin-full">One image is shared by the Bookings, Services and Hire pages for a consistent look.</p><button className="admin-button" type="submit" disabled={saving}>{saving ? 'Uploading…' : 'Update shared hero'}</button></form></Panel><Panel title="Current shared hero"><div className="page-artwork-single"><img src={sharedHero} alt="Current Bookings, Services and Hire hero" /></div></Panel></div></section>
}
