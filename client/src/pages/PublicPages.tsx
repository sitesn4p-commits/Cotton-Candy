...
function HeroTypedHeading({ onPointerDown, firstLine = 'Make your day', secondLine = 'unforgettably beautiful.' }: { onPointerDown: (event: ReactPointerEvent<HTMLHeadingElement>) => void; firstLine?: string; secondLine?: string }) {
  ...
  return <h1 className="hero-flower-heading hero-typed-heading" onPointerDown={onPointerDown}><span className="hero-typed-line hero-typed-line-primary">{firstLine.slice(0, firstCharacters)}</span><br /><em className="hero-typed-line hero-typed-line-accent">{secondLine.slice(0, secondCharacters)}</em>{typedCharacters < firstLine.length + secondLine.length ? <span className="hero-typing-caret" aria-hidden="true" /> : null}</h1>
}
...
function CollectionHeroArtwork({ type }: { type: CollectionType }) {
  if (type === 'service') return <div className="collection-hero-art service-hero-art" aria-hidden="true"><span className="service-orbit service-orbit-one" /><span className="service-orbit service-orbit-two" /><span className="service-spark service-spark-one">✦</span><span className="service-spark service-spark-two">✦</span><div className="service-mood-card"><span className="service-card-tape service-card-tape-one" /><span className="service-card-tape service-card-tape-two" /><p>Styled with intention</p><strong>Dream it.<br />We make it <em>pretty.</em></strong><div className="service-card-lines"><i /><i /><i /></div><div className="service-colour-swatches"><span /><span /><span /><span /></div></div><div className="service-ribbon"><span>made for your moment</span></div></div>
  return <div className="collection-hero-art hire-hero-art" aria-hidden="true"><span className="hire-sun" /><span className="hire-spark hire-spark-one">✦</span><span className="hire-spark hire-spark-two">✦</span><div className="hire-backdrop"><span className="hire-backdrop-dot hire-backdrop-dot-one" /><span className="hire-backdrop-dot hire-backdrop-dot-two" /></div><div className="hire-plinth hire-plinth-tall"><span className="hire-vase"><i /><i /><i /></span></div><div className="hire-plinth hire-plinth-short"><span className="hire-bow" /></div><div className="hire-sign"><span>your<br /><em>day</em></span></div></div>
}
...
function CustomerOrderManager({ order, email, onSaved, onClose }: { order: ServiceRequest; email: string; onSaved: (request: ServiceRequest) => void; onClose: () => void }) {
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await api.updateCustomerOrder({ action: 'update', email, referenceId, customerName, phone, eventType, eventDate, hireDays, notes })
  ...
  const cancel = async () => {
    ...
    setSaving(true); setError(null)
    try {
      const response = await api.updateCustomerOrder({ action: 'cancel', email, referenceId })
      onSaved(response.request)
...
function RequestTracker() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); setError(null); setOrders([]); setManaging(null); setSearched(false); setSearching(true)
    try { setOrders(await api.ordersByEmail(email.trim())); setSearched(true) } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Unable to find orders for this email address.') } finally { setSearching(false) }
  }
