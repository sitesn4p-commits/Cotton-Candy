...
export const api = {
  ...
  registerAdminPushDevice: (token: string, body: { token: string; platform: 'web' | 'android' | 'desktop'; userAgent?: string }) => request<void>('/admin/push-devices', { method: 'POST', body, token }),
  unregisterAdminPushDevice: (token: string, deviceToken: string) => request<void>('/admin/push-devices', { method: 'DELETE', body: { token: deviceToken }, token }),
